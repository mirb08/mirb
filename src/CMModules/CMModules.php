<?php
/**
 * A model for managing Mirb modules.
 * 
 * @package MirbCore
 */
class CMModules extends CObject {
   ...
   * Properties
   */
  private $mirbCoreModules = array('CMirb', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
  private $mirbCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
  ...
  private function GetDetailsOfModule($module) {
    ...
    if(class_exists($module)) {
      ...
      $details['isMirbCore']   = in_array($rc->name, $this->mirbCoreModules);
      $details['isMirbCMF']    = in_array($rc->name, $this->mirbCMFModules);
