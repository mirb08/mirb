...
function get_debug() {
  ...
  if(isset($mi->config['debug']['mirb']) && $mi->config['debug']['mirb']) {
    $html .= "<hr><h3>Debuginformation</h3><p>The content of CMirb:</p><pre>" . htmlent(print_r($mi, true)) . "</pre>";
  }    
  if(isset($mi->config['debug']['session']) && $mi->config['debug']['session']) {
    $html .= "<hr><h3>SESSION</h3><p>The content of CMirb->session:</p><pre>" . htmlent(print_r($mi->session, true)) . "</pre>";
...
function get_messages_from_session() {
  $messages = CMirb::Instance()->session->GetMessages();
  $html = null;
  if(!empty($messages)) {
...
function get_gravatar($size=null) {
  return 'http://www.gravatar.com/avatar/' . md5(strtolower(trim(CMirb::Instance()->user['email']))) . '.jpg?r=pg&amp;d=wavatar&amp;' . ($size ? "s=$size" : null);
}
...
function base_url($url=null) {
  return CMirb::Instance()->request->base_url . trim($url, '/');
}
...
function create_url($urlOrController=null, $method=null, $arguments=null) {
  return CMirb::Instance()->CreateUrl($urlOrController, $method, $arguments);
}
...
function theme_url($url) {
  return create_url(CMirb::Instance()->themeUrl . "/{$url}");
}
...
function theme_parent_url($url) {
  return create_url(CMirb::Instance()->themeParentUrl . "/{$url}");
}
...
function current_url() {
  return CMirb::Instance()->request->current_url;
}
...
function render_views($region='default') {
  return CMirb::Instance()->views->Render($region);
}
...
function region_has_content($region='default' /*...*/) {
  return CMirb::Instance()->views->RegionHasView(func_get_args());
}
