...
class CMUser extends CObject implements IHasSQL, ArrayAccess, IModule {
    ...
    switch($action) {
      case 'install': 
        try {
          ...
          $this->db->ExecuteQuery(self::SQL('insert into user'), array('root', 'The Administrator', 'mirb08@student.bth.se', $password['algorithm'], $password['salt'], $password['password']));
  ...
  public function CreatePassword($plain, $algorithm=null) {
    $password = array(
      'algorithm'=>($algorithm ? $algoritm : CMirb::Instance()->config['hashing_algorithm']),
      'salt'=>null
    );
