...
function autoload($aClassName) {
  $classFile = "/src/{$aClassName}/{$aClassName}.php";
	$file1 = MIRB_SITE_PATH . $classFile;
	$file2 = MIRB_INSTALL_PATH . $classFile;
	if(is_file($file1)) {
		require_once($file1);
...
function exceptionHandler($e) {
  echo "Mirb: Uncaught exception: <p>" . $e->getMessage() . "</p><pre>" . $e->getTraceAsString(), "</pre>";
}
...
function htmlEnt($str, $flags = ENT_COMPAT) {
  return htmlentities($str, $flags, CMirb::Instance()->config['character_encoding']);
}
